(function () {
  ...
  const VERSION = '20260922-direct5-original-rules';
  ...
  function installStyles() {
    let style = document.getElementById('hashcodDirectRegistrationStyles');
      ...
      '<div class="hashcod-direct-card" role="dialog" aria-modal="true" aria-label="Registro Hashcod Codespace">',
        ...
        '<form id="hashcodDirectRegistrationForm" novalidate autocomplete="off" data-no-autosave data-hashcod-autosave="off">',
          ...
          '<div class="hashcod-registration-actions">',
            ...
            '<div class="hashcod-registration-validity-note"><svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 32 32" aria-hidden="true"><path d="M 9 5 L 9 7 L 23 7 L 23 5 L 9 5 z M 23 7 L 23 10 L 25 10 L 25 7 L 23 7 z M 25 10 L 25 14 L 27 14 L 27 10 L 25 10 z M 27 14 L 27 17 L 5 17 L 5 14 L 3 14 L 3 27 L 29 27 L 29 14 L 27 14 z M 5 14 L 7 14 L 7 10 L 5 10 L 5 14 z M 7 10 L 9 10 L 9 7 L 7 7 L 7 10 z M 12 9 L 12 11 L 20 11 L 20 9 L 12 9 z M 20 11 L 20 13 L 22 13 L 22 11 L 20 11 z M 20 13 L 17 13 L 17 12 L 15 12 L 15 13 L 12 13 L 12 15 L 20 15 L 20 13 z M 12 13 L 12 11 L 10 11 L 10 13 L 12 13 z M 5 19 L 27 19 L 27 25 L 5 25 L 5 19 z M 23 21 L 23 23 L 25 23 L 25 21 L 23 21 z"></path></svg><span>Haciendo que tu proyecto hecho por IA tenga validez legal</span></div>',
          '</div>',
  ...
  function getValues() {
    return {
      full_name: field('hashcodDirectFullName'),
      age: field('hashcodDirectAge'),
      cedula: field('hashcodDirectCedula'),
      platform_name: field('hashcodDirectPlatform'),
      code_file: field('hashcodDirectCodeFile'),
      email: field('hashcodDirectEmail'),
      phone: field('hashcodDirectPhone'),
      consent: field('hashcodDirectConsent')
    };
  }
  ...
  function validate() {
    const f = getValues();
    const v = validity();
    const rules = Object.keys(v);
    const done = rules.filter(function (key) { return v[key]; }).length;
    const percent = Math.round((done / rules.length) * 100);
    const progress = field('hashcodDirectProgress');
    const indicator = field('hashcodDirectProgressIndicator');
    const value = field('hashcodDirectProgressValue');
    const hint = field('hashcodDirectProgressHint');
    const submit = field('hashcodDirectSubmit');
    const whatsapp = field('hashcodDirectWhatsapp');
    ...
    if (f.full_name) f.full_name.classList.toggle('is-invalid', f.full_name.value.length > 0 && !v.full_name);
    if (f.age) f.age.classList.toggle('is-invalid', f.age.value.length > 0 && !v.age);
    if (f.cedula) f.cedula.classList.toggle('is-invalid', f.cedula.value.length > 0 && !v.cedula);
    if (f.platform_name) f.platform_name.classList.toggle('is-invalid', f.platform_name.value.length > 0 && !v.platform_name);
    if (f.email) f.email.classList.toggle('is-invalid', f.email.value.length > 0 && !v.email);
    if (f.phone) f.phone.classList.toggle('is-invalid', f.phone.value.length > 0 && !v.phone);
  ...
  function generateCode() {
    ...
    return 'HSC-REG-' + Array.from(bytes).map(function (b) { return b.toString(16).padStart(2, '0').toUpperCase(); }).join('-').replace(/-/g, '').replace(/(.{4})/g, '$1-').replace(/-$/, '');
  }
  ...
  function whatsappMessage() {
    ...
    return [
      ...
      'Nombre: ' + (f.full_name ? f.full_name.value.trim() : ''),
      'Edad: ' + (f.age ? f.age.value.trim() : ''),
      'Cédula: ' + (f.cedula ? f.cedula.value.trim() : ''),
      'Plataforma: ' + (f.platform_name ? f.platform_name.value.trim() : ''),
  ...
  function bindDirectRegistration() {
    const f = getValues();
    const form = field('hashcodDirectRegistrationForm');
    const status = field('hashcodDirectStatus');
    const codeButton = field('hashcodDirectCodeButton');
    const codeFile = field('hashcodDirectCodeFile');
    const whatsapp = field('hashcodDirectWhatsapp');
    const temporary = field('hashcodDirectTemporary');
    const back = field('hashcodDirectBack');

    Object.keys(f).forEach(function (key) {
      const input = f[key];
      if (!input || input.dataset.boundDirectRegistration) return;
      input.dataset.boundDirectRegistration = 'true';
    ...
    if (codeButton && codeFile && !codeButton.dataset.boundDirectRegistration) {
      codeButton.dataset.boundDirectRegistration = 'true';
    ...
    document.querySelectorAll('#hashcodDirectRegistration .hashcod-registration-faq-trigger').forEach(function (button) {
      if (button.dataset.boundAccordion) return;
      button.dataset.boundAccordion = 'true';
    ...
    if (form && !form.dataset.boundDirectRegistration) {
      form.dataset.boundDirectRegistration = 'true';
      form.addEventListener('submit', function (event) {
        ...
        removeNode('hashcodDirectCodeReceipt');
        document.body.insertAdjacentHTML('beforeend', receiptMarkup(code));
        const copy = field('hashcodDirectCopyCode');
        const cont = field('hashcodDirectContinueAfterCode');
        const copyStatus = field('hashcodDirectCodeCopyStatus');
        if (copy) copy.addEventListener('click', function () {
          navigator.clipboard && navigator.clipboard.writeText(code).then(function () {
            ...
            if (copyStatus) copyStatus.textContent = 'Copia manualmente el código.';
          });
        });
    ...
    if (whatsapp && !whatsapp.dataset.boundDirectRegistration) {
      whatsapp.dataset.boundDirectRegistration = 'true';
    ...
    if (temporary && !temporary.dataset.boundDirectRegistration) {
      temporary.dataset.boundDirectRegistration = 'true';
      temporary.addEventListener('click', openTemporaryAccessDialog);
    }

    if (back && !back.dataset.boundDirectRegistration) {
      back.dataset.boundDirectRegistration = 'true';
  ...
  function openTemporaryAccessDialog() {
    removeNode('hashcodDirectTemporaryDialog');
    document.body.insertAdjacentHTML('beforeend', temporaryDialogMarkup());
    const dialog = field('hashcodDirectTemporaryDialog');
    const cancel = field('hashcodTemporaryAccessCancel');
    const confirm = field('hashcodTemporaryAccessConfirm');
    const status = field('hashcodTemporaryAccessStatus');
    if (dialog) dialog.classList.add('is-open');
    if (cancel) cancel.addEventListener('click', function () { removeNode('hashcodDirectTemporaryDialog'); });
    if (confirm) confirm.addEventListener('click', function () {
      const amount = Math.max(1, Number(field('hashcodTemporaryAccessDuration') && field('hashcodTemporaryAccessDuration').value) || 30);
      const unit = field('hashcodTemporaryAccessUnit') && field('hashcodTemporaryAccessUnit').value === 'hours' ? 'hours' : 'minutes';
      const ms = Math.min(unit === 'hours' ? amount * 60 * 60 * 1000 : amount * 60 * 1000, 24 * 60 * 60 * 1000);
      temporaryAccessExpiresAt = Date.now() + ms;
      if (status) status.textContent = 'Acceso temporal concedido.';
      if (temporaryAccessTimer) clearTimeout(temporaryAccessTimer);
      temporaryAccessTimer = setTimeout(function () { window.location.reload(); }, ms);
      finishEntry();
    });
  }
  ...
  function openDirectRegistration() {
    ...
    const firstInput = field('hashcodDirectFullName');
