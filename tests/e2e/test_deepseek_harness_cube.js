...
assert(loader.includes('deepseek-harness-cube.css?v=20260916-1'), 'DSH loader must load cube styling');
assert(loader.includes('deepseek-harness-cube.js?v=20260916-1'), 'DSH loader must load cube runtime');
assert(loader.includes('data-hashcod-deepseek-harness-cube'), 'DSH loader idempotency marker missing');
assert(sharedLoader.includes('deepseek-harness-loader.js?v=20260916-1'), 'shared Render/Laragon layer must load the DSH loader');
...
console.log('PASS: DeepSeek Harness owns the sixth tray cube, stays loopback-only, and loads through the shared platform layer.');