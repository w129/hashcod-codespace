...
<head>
    ...
    <title>macOS inside · l8 codespace</title>
    ...
    <style>
        ...
        * { box-sizing: border-box; }
        ...
        .shell {
            ...
            min-height: 100vh;
            background:
                radial-gradient(ellipse 90% 60% at 50% 0%, rgba(41, 151, 255, 0.12), transparent 55%),
                linear-gradient(180deg, #1a1a1c 0%, #0d0d0f 45%, #050506 100%);
        }
        .menubar {
            display: flex;
            align-items: center;
        ...
        <div class="menubar">
            <div class="brand">
                <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" aria-hidden="true">
                    <path d="M16.365 1.43c0 1.14-.837 2.445-1.86 3.215-1.01.76-2.15.99-2.15.99s-.05-1.24.86-2.52c.9-1.27 2.17-1.96 3.15-1.685zM19.94 17.17c-.63 1.45-.93 2.1-1.74 3.39-1.13 1.78-2.43 4-4.19 4.01-1.57.02-1.97-1.02-4.11-1.01-2.14.01-2.58 1.03-4.15 1.01-1.76-.02-3.11-2.02-4.24-3.8C-.01 17.8-.79 13.12.98 10.3c1.13-1.79 2.92-2.84 4.61-2.84 1.72 0 2.8 1.05 4.22 1.05 1.38 0 2.22-1.05 4.21-1.05 1.5 0 3.09.82 4.21 2.23-3.7 2.03-3.1 7.31.71 7.48z"/>
                </svg>
                <div>
                    <h1>macOS inside</h1>
                    <span>dockur/macos · l8 codespace</span>
                </div>
            </div>
        ...
        <div class="stage visible" id="stageSystem">
            <div class="term-wrap">
                ...
                <form class="input-row" id="termForm" autocomplete="off">
                    <div class="ps1" id="ps1"><span class="user">macos@l8</span>:<span class="path">~</span>$</div>
                    <input id="termIn" type="text" spellcheck="false" autofocus aria-label="macOS command">
                </form>
            </div>
        </div>
        ...
        <div class="stage" id="stageLicense">
            <div class="license-wrap">
                <div class="license-card">
                    <h2>License</h2>
                    <p class="sub">Licencia del sistema operativo desplegado desde <code>https://github.com/dockur/macos</code> (archivo <code>license.md</code> del recurso).</p>
                    ...
                    <div class="license-actions">
                        <a href="https://github.com/dockur/macos/blob/master/license.md" target="_blank" rel="noopener noreferrer">Ver en GitHub</a>
    ...
    <script>
        (function () {
            ...
            function setPrompt(path) {
                cwd = path || cwd;
                ps1.innerHTML = '<span class="user">macos@l8</span>:<span class="path">' + escapeHtml(cwd) + '</span>$';
            }
            ...
            async function loadLicense() {
                ...
                try {
                    const res = await fetch(l8ApiUrl('api/macos/license'));
            ...
            async function boot() {
                append('<div class="boot-line">macOS inside · l8 codespace</div>');
                append('<div class="boot-line">Loading resources from github.com/dockur/macos …</div>');
                try {
                    const res = await fetch(l8ApiUrl('api/macos/session'), {
                        method: 'POST',
                    ...
                    meta.textContent = (data.distro || 'macOS') + ' · ' + (data.resource || 'dockur/macos');
            ...
            async function runCommand(cmd) {
                ...
                if (cmd === 'license' || cmd === 'licence') {
                    append('<div><span class="prompt">macos@l8</span>:<span class="path">' + escapeHtml(cwd) + '</span>$ <span class="cmd">' + escapeHtml(cmd) + '</span></div>');
                    showStage('license');
                    await loadLicense();
                    return;
                }
                ...
                append('<div><span class="prompt">macos@l8</span>:<span class="path">' + escapeHtml(cwd) + '</span>$ <span class="cmd">' + escapeHtml(cmd) + '</span></div>');
                try {
                    const res = await fetch(l8ApiUrl('api/macos/exec'), {
                        method: 'POST',
                        headers: { 'Content-Type': 'application/json' },
