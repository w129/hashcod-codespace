 ...
 * Loaded by index.php / index.html; exposes openToolkitPdfMd / initToolkitPdfMd.
 */
(function () {
    'use strict';
    ...
    function toolkitPdfProcessInWorker(arrayBuffer) {
        ...
        const source =
            'import init, { processPdf, version } from "' + wasmUrl + '";\n' +
            'self.onmessage = async ({ data }) => {\n' +
            '  try {\n' +
            '    await init();\n' +
            '    const result = processPdf(new Uint8Array(data.buffer), { profile: "fidelity" });\n' +
            '    self.postMessage({ ok: true, result: result, engineVersion: version() });\n' +
            '  } catch (error) {\n' +
    ...
    function toolkitPdfRenderResult(payload) {
        const result = (payload && payload.result) || {};
        toolkitPdfMarkdown = typeof result.markdown === 'string' ? result.markdown : '';
        ...
        const type = result.pdfType || result.pdf_type || '—';
        const pages = result.pageCount != null ? result.pageCount : (result.page_count != null ? result.page_count : '—');
        const ms = Number.isFinite(result.processingTimeMs)
            ? Math.max(1, Math.round(result.processingTimeMs)) + ' ms'
            : (Number.isFinite(result.processing_time_ms)
                ? Math.max(1, Math.round(result.processing_time_ms)) + ' ms'
                : '—');
        const ver = payload.engineVersion ? ' · motor v' + payload.engineVersion : '';

        if (meta) {
            meta.textContent = type + ' · ' + pages + ' pág. · ' + ms + ver;
        }
        if (out) out.textContent = toolkitPdfMarkdown || '(Sin markdown — PDF escaneado o sin capa de texto)';
        if (empty) empty.style.display = 'none';
        if (preview) preview.hidden = false;
        if (copyBtn) copyBtn.disabled = !toolkitPdfMarkdown;
        if (dlBtn) dlBtn.disabled = !toolkitPdfMarkdown;

        if (toolkitPdfMarkdown && typeof window.toolkitEnsureMarked === 'function') {
            window.toolkitEnsureMarked().then((marked) => {
                const htmlView = document.getElementById('toolkitPdfHtml');
                if (htmlView && marked && typeof marked.parse === 'function') {
    ...
    async function toolkitPdfConvertFile(file) {
        ...
        try {
            ...
            const payload = await toolkitPdfProcessInWorker(buffer);
            toolkitPdfRenderResult(payload);
            toolkitPdfSetStatus('Convertido localmente con pdf-inspector.', 'ok');
            if (typeof window.toolkitLogUse === 'function') {
                window.toolkitLogUse('platform', 'pdf-md', 'PDF→MD · ' + toolkitPdfFileName);
            }
            if (typeof window.toolkitCurateFile === 'function') {
