...
<head>
    ...
    <style>
        ...
        * {
        ...
        .st-dock-btn.secondary { background: #111; }
        .st-dock-btn.ghost {
            background: #fff;
            color: #111;
            border: 1px solid #E5E7EB;
...
`;
        ...
        function stDockSetBadges(status, tool) {
            ...
            const bits = [];
            bits.push('<span class="st-dock-badge ' + (avail ? 'on' : 'off') + '">' +
                (avail ? ('Streamlit' + (status.version ? ' ' + status.version : ' listo')) : 'Streamlit no instalado') +
                '</span>');
            bits.push('<span class="st-dock-badge ' + (running ? 'on' : 'off') + '">' +
                (running ? 'En ejecución' : 'Detenida') + '</span>');
            bits.push('<span class="st-dock-badge">' + (hasCode ? 'Con código' : 'Vacío') + '</span>');
            if (tool && tool.template) {
                bits.push('<span class="st-dock-badge">Plantilla · ' + String(tool.template).replace(/</g, '') + '</span>');
            }
        ...
        function stDockShowPreview(url) {
            ...
            iframe.src = url;
            const btn = document.getElementById('stDockPreviewBtn');
            if (btn) btn.textContent = 'Ocultar vista';
        }
        ...
        async function stDockLoadSlot(slot) {
            ...
            stDockSetMsg(
                data.running
                    ? 'App en ejecución. Ábrela, previsualiza o detén.'
                    : (data.has_code ? 'Código listo. Guarda y abre para desplegarla.' : 'Slot vacío: elige una plantilla o pega código.'),
    ...
    <div class="st-dock-overlay" id="stDockOverlay" aria-hidden="true">
        <div class="st-dock-shell" role="dialog" aria-modal="true" aria-labelledby="stDockTitle">
            ...
            <div class="st-dock-label">Plantillas</div>
            <div class="st-dock-templates" id="stDockTemplates" aria-label="Plantillas Streamlit"></div>
            ...
            <div class="st-dock-actions">
                <button type="button" class="st-dock-btn ghost" id="stDockSaveBtn">Guardar</button>
                ...
                <button type="button" class="st-dock-btn ghost" id="stDockPreviewBtn">Vista previa</button>
                <button type="button" class="st-dock-btn ghost" id="stDockStopBtn">Detener</button>
                <button type="button" class="st-dock-btn danger" id="stDockClearBtn">Vaciar slot</button>
            </div>
            <div class="st-dock-preview-wrap" id="stDockPreviewWrap">
                <iframe id="stDockIframe" title="Vista previa Streamlit" loading="lazy"></iframe>
            </div>
            <p class="st-dock-msg" id="stDockMsg">Elige una plantilla o pega tu app. Ctrl/Cmd+Enter guarda y abre.</p>
        </div>
    </div>
