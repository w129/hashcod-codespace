...
`;
    ...
    // ===== ON-DEMAND MODULES CATALOG & WHATSAPP DISPATCHER =====
    const ON_DEMAND_CATALOG = {
        'git-vault': {
            id: 'git-vault',
            name: 'Git Repository Vault & Sync Engine',
            code: 'HASHCOD-GIT-08',
            slot: 'Círculo #8 (Toolbox)',
            badge: 'ON-DEMAND BUILD',
            color: '#F4511E',
            iconSvg: `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 48 48" width="40" height="40"><path fill="#F4511E" d="M42.2,22.1L25.9,5.8C25.4,5.3,24.7,5,24,5c0,0,0,0,0,0c-0.7,0-1.4,0.3-1.9,0.8l-3.5,3.5l4.1,4.1c0.4-0.2,0.8-0.3,1.3-0.3c1.7,0,3,1.3,3,3c0,0.5-0.1,0.9-0.3,1.3l4,4c0.4-0.2,0.8-0.3,1.3-0.3c1.7,0,3,1.3,3,3s-1.3,3-3,3c-1.7,0-3-1.3-3-3c0-0.5,0.1-0.9,0.3-1.3l-4-4c-0.1,0-0.2,0.1-0.3,0.1v10.4c1.2,0.4,2,1.5,2,2.8c0,1.7-1.3,3-3,3s-3-1.3-3-3c0-1.3,0.8-2.4,2-2.8V18.8c-1.2-0.4-2-1.5-2-2.8c0-0.5,0.1-0.9,0.3-1.3l-4.1-4.1L5.8,22.1C5.3,22.6,5,23.3,5,24c0,0.7,0.3,1.4,0.8,1.9l16.3,16.3c0,0,0,0,0,0c0.5,0.5,1.2,0.8,1.9,0.8s1.4-0.3,1.9-0.8l16.3-16.3c0.5-0.5,0.8-1.2,0.8-1.9C43,23.3,42.7,22.6,42.2,22.1z"></path></svg>`,
            description: 'Módulo de sincronización y control de versiones distribuido con firma criptográfica PQC, automatización de commits, soporte para repositorios remotos y despliegue de pipelines CI/CD a medida.',
            delivery: 'Fabricación a Medida (24-48h)'
        }
    };
    ...
    function openOnDemandToolModal(toolId) {
        ...
        const modal = document.getElementById('onDemandToolModal');
        if (!modal) return;

        const titleEl = document.getElementById('odModalTitle');
        const nameEl = document.getElementById('odModalToolName');
        const codeEl = document.getElementById('odModalItemCode');
        const descEl = document.getElementById('odModalDescription');
        const badgeEl = document.getElementById('odModalBadge');
        const deliveryEl = document.getElementById('odModalDelivery');
        const slotEl = document.getElementById('odModalSlotName');
        const iconContainer = document.getElementById('odModalIconContainer');

        if (titleEl) titleEl.textContent = `Krumbs Hub — ${tool.name}`;
        if (nameEl) nameEl.textContent = tool.name;
        if (codeEl) codeEl.textContent = tool.code;
        if (descEl) descEl.textContent = tool.description;
        if (badgeEl) badgeEl.textContent = tool.badge;
        if (deliveryEl) deliveryEl.textContent = tool.delivery;
        if (slotEl) slotEl.textContent = tool.slot;
        if (iconContainer) iconContainer.innerHTML = tool.iconSvg;
    ...
    function sendOnDemandWhatsAppOrder() {
        ...
        const messageText = `╔═════════════════════════════════════════════════════════╗\n║   HASHCOD CODESPACE — SOLICITUD DE ARTÍCULO A MEDIDA    ║\n╚═════════════════════════════════════════════════════════╝\n\n*Artículo Solicitado:* ${tool.name}\n*Código Ref:* \`${tool.code}\`\n*Ubicación:* ${tool.slot}\n*Voucher:* \`${refId}\`\n*Fecha:* ${timestamp}\n\nHola, deseo hacer el pedido y cotizar la fabricación de este módulo de software para mi proyecto en Hashcod Codespace.`;
